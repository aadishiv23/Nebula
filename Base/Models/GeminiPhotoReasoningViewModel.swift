            ...
            for item in selectedItems {
                if let data = try? await item.loadTransferable(type: Data.self) {
                    guard let image = UIImage(data: data) else {
                        logger.error("Failed to parse data as image")
                        continue
                    }
                    if image.size.fits(largestDimension: GeminiPhotoReasoningViewModel.largestImageDimension) {
                        images.append(image)
                    } else {
                        guard let resizedImage = image
                            .preparingThumbnail(of: image.size
                                .aspectFit(largestDimension: GeminiPhotoReasoningViewModel.largestImageDimension)) else {
